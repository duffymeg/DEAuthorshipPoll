...
save_plot("lastplotgrid4panelv2.jpg", lastplotgrid4panelv2, base_width = 9, base_height = 15)
```
...
save_plot("lastplotgrid2panelv2.jpg", lastplotgrid2panelv2, base_width = 9, base_height = 10)
```
...
currentsummary <- corrcurrdata %>%
  group_by(PhD01,CorrespondingCurrentShort) %>%
  summarise(n=n()) %>%
  mutate(rel.freq = round(100 * n/sum(n), 0))
...
currentlineplot <- currentsummary %>%
  ...
  filter(PhD01 < 7) %>%
  ggplot(aes(x=PhD01,y=rel.freq,color= CorrespondingCurrentShort)) +
...
Journal = c("Evolution","Evolution","Evolution","Evolution","Evolution")
Year = c(1956,1966,1976,1986,1996)
Number.authors = c(-10,-10,-10,-10,-10)
...
save_plot("threepanellineplot.jpg", threepanellineplot, base_width = 8, base_height = 4)
```
